...
- 基于 RDKit 自动生成 100+ 分子描述符，支持 Van Krevelen 基团贡献和自定义特征公式。
- 一键运行 12 种机器学习模型（Ridge、XGBoost、Gaussian Process 等），Optuna 自动超参优化，K-Fold / LOOCV 交叉验证。
- 可视化预测-实际散点图、残差分析、模型对比和特征重要性。
- 保存训练好的模型，对新聚合物 SMILES 进行性质预测；GPR 返回后验标准差，其他模型明确返回外部验证 RMSE。
...
Windows 用户可在 GitHub 仓库的 **[Releases](https://github.com/YueQiandesedianzi/PolyML/releases)** 页面下载 `PolyML Setup 0.1.1.exe`。

当前安装包未进行商业代码签名。Windows SmartScreen 如显示未知发布者，请核对 Release 页面提供的信息后再决定是否运行。

安装包会携带 FastAPI 后端源码，但 v0.1.1 仍需要本机 Conda 环境。应用按 `POLYML_PYTHON`、当前 `CONDA_PREFIX`、名为 `polyml` 的 Conda 环境、常见 Miniconda/Anaconda 路径依次寻找 Python。若环境不存在，请先执行：

```powershell
conda env create -f backend/environment.yml
```

Windows 发布复现可使用本次验证环境的直接依赖锁：`backend/environment-win-64.lock.yml`。
...
- 训练、保存和预测统一使用带特征规格、目标单位和模型卡的 `ModelBundleV2`；旧模型标记为 legacy，必须重新训练。
- 模型按训练集内部交叉验证选型，外部测试集只评估最终最佳模型；支持随机、分组和时间划分。
- DOE 使用 pyDOE3 的标准部分因子、Box–Behnken、CCD 和 LHS；未测量方案进入独立实验队列。
- 主动学习只从 DOE 或导入的未标注候选集中推荐，不会重新推荐训练样本。
- 代码导出同时提供可执行 Python 脚本和可移植 joblib 模型包。
- Electron 与本机 FastAPI 使用每次启动随机生成的会话令牌通信。
...
- 分子描述符计算依赖有效的 SMILES 字符串；无效 SMILES 会被记录，并由训练管道按缺失特征处理。
