...
class BayesianOptimizer:
    ...
    def __init__(
        ...
        self.X_labeled = self.scaler.fit_transform(X_labeled)
    ...
    def suggest_next(
        ...
        for idx in top_idx:
            suggestions.append({
                ...
                "predicted_y": float(self.gpr.predict(self.scaler.transform(X_candidates[idx:idx+1]))[0]),
                "predicted_std": float(
                    self.gpr.predict(self.scaler.transform(X_candidates[idx:idx+1]), return_std=True)[1][0]
                ),
            })
    ...
    def cross_validate(self, n_folds: int = 5) -> dict:
        ...
        kf = KFold(n_splits=min(n_folds, len(self.X_labeled)), shuffle=True, random_state=42)
        y_true_all, y_pred_all = [], []

        for train_idx, val_idx in kf.split(self.X_labeled):
            gpr = GaussianProcessRegressor(kernel=self.kernel, normalize_y=True)
            gpr.fit(self.X_labeled[train_idx], self.y_labeled[train_idx])
            y_pred = gpr.predict(self.X_labeled[val_idx])
